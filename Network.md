...
- **3-way 핸드셰이크** ( `three-way handshake` ) : TCP 통신에서 사용하는 **신뢰성**을 제공하기 위한 통신 방식이다. 컴퓨터 간에 연결을 맺기 위한 초기화 과정이며 세 단계로 되어 있어서 `three-way` 라고 부른다.  ( [출처](http://www.yes24.com/Product/Goods/61794014) )   

- **잘 알려진 포트** ( `well-known prots` ) : 특정 애플리케이션이 사용할 수 있도록 예약되어 있는 포트로, 1~1023번 포트를 말한다.  ( [출처](http://www.yes24.com/Product/Goods/61794014) )

- **일련번호** ( `sequence number` ) : TCP에서는 데이터를 보낼 때마다 **각 데이터에 고유한 번호**를 부여해서 전송을 시도한다. 이 번호를 이용하여 **TCP 패킷의 순서를 제어**할 수 있다.  ( [출처](http://www.yes24.com/Product/Goods/61794014) )

- **응용 계층** ( `application layer`, 애플리케이션 계층 ) : OSI 모델의 최상위 계층으로, 다양하게 존재하는 응용 환경에서 공통적으로 필요한 기능을 다룬다. 이메일, 파일 전송, 웹 사이트 조회 등 애플리케이션에 대한 서비스를 제공하는 계층이다.  ( [출처](http://www.yes24.com/Product/Goods/61794014) )

- **WWW** ( `World Wide Web`, 월드 와이드 웹 ) : 통신 회선이 거미줄처럼 서로 연결 되어 있어서 언제 어디서든 필요한 곳에 접근하여 정보를 주고 받을 수 있는 멀티미디어 인터넷 서버이다.  ( [출처](http://www.yes24.com/Product/Goods/61794014) )
